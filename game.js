(function () {
  ...
  function step() {
    ...
    // Self collision — skip last segment since it will vacate
  ...
  canvas.addEventListener('touchend', function (e) {
    ...
    if (Math.max(adx, ady) < 20) {
      // Short tap — start or toggle pause
      if (gameState === 'start' || gameState === 'gameover') startGame();
      else togglePause();
      return;
    }
  ...
  document.querySelectorAll('.dpad-btn').forEach(function (btn) {
    ...
    btn.addEventListener('mousedown', function (e) { e.preventDefault(); }); // no focus steal
  });
  ...
  // ============================================================
  // Boot — wait for fonts so canvas text renders correctly
  // ============================================================
  hiEl.textContent = highScore;
  syncUI();
